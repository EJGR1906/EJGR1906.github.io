        ...
        <AppShell activeItem="Configuración" onNavigate={onNavigate}>
            <div className="p-4 sm:p-6 lg:p-8"><div className="mx-auto max-w-4xl">
                ...
                <div className="grid gap-6 lg:grid-cols-2">
                    ...
                    <section className="rounded-3xl bg-white p-5 shadow-sm ring-1 ring-primary/5 sm:p-6"><div className="flex items-center gap-2"><SettingsIcon size={19} className="text-primary" /><h2 className="font-bold text-primary-dark">Respaldo local</h2></div><p className="mt-2 text-sm text-primary-dark/60">Exporta tus tablas para conservar una copia o restaurarlas en este dispositivo.</p><div className="mt-4 flex flex-wrap gap-2"><button type="button" onClick={() => void handleExport()} className="rounded-xl bg-primary px-4 py-2.5 text-sm font-semibold text-white">Exportar JSON</button><button type="button" onClick={() => void handleCsvExport()} className="rounded-xl border border-primary/15 px-4 py-2.5 text-sm font-semibold text-primary-dark">Exportar movimientos CSV</button><label className="cursor-pointer rounded-xl border border-primary/15 px-4 py-2.5 text-sm font-semibold text-primary-dark">Importar JSON<input type="file" accept="application/json,.json" onChange={(event) => void handleImport(event)} className="sr-only" /></label></div></section>
                </div>
            </div></div>
        </AppShell>
    );
