...
import { db, type Category, type CurrencyCode } from "../database/db";
import { getDashboardSummary } from "../services/dashboardService";
import { getBaseCurrency } from "../services/settingsService";

const storageKey = "finanzas.budgets";

type Budget = { categoryId: string; amount: number; currency: CurrencyCode };
...
function Budgets() {
    ...
    useEffect(() => {
        const load = async () => {
            const [categoryData, summary] = await Promise.all([
                ...
                getDashboardSummary(baseCurrency, "month"),
            ]);
    ...
    const saveBudget = (event: React.FormEvent) => {
        ...
        const next = [...budgets.filter((budget) => budget.categoryId !== categoryId), { categoryId, amount: value, currency: baseCurrency }];
        setBudgets(next);
                    ...
                    <div className="grid gap-6 lg:grid-cols-[0.8fr_1.2fr]">
                        <section className="rounded-3xl bg-white p-5 shadow-sm ring-1 ring-primary/5 sm:p-6">
                            ...
                            <form onSubmit={saveBudget} className="space-y-4">
                                ...
                                <label className="block text-sm font-medium text-primary-dark">Límite mensual en {baseCurrency}<input className="field mt-1" type="number" min="1" step="0.01" value={amount} onChange={(event) => setAmount(event.target.value)} placeholder="300" /></label>
                        ...
                        <section className="rounded-3xl bg-white p-5 shadow-sm ring-1 ring-primary/5 sm:p-6">
                            <div className="flex items-center gap-2"><WalletCards size={19} className="text-primary" /><h2 className="font-bold text-primary-dark">Este mes · {baseCurrency}</h2></div>
